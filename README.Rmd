...
text_scores <- prsp_score(
           trump_tweet, 
           key = key, score_sentences = T,
           score_model = peRspective::prsp_models
           )

text_scores %>% 
  select(type, score, text) %>% 
  gather(key, value, -text, -score) %>% 
  mutate(value = fct_reorder2(value, text, score)) %>% 
...
dat <- data.frame(date = c(as.Date("2019-01-02"),
                           as.Date("2019-01-01"),
                           as.Date("2019-01-03")))

dat$date <- as.numeric(as.factor(dat$date))
```
...
text <- "Con la llegado de internet y de las nuevas tecnologías de la información, la forma de contactar que tenemos entre los seres humanos ha cambiado y lo va a seguir haciendo en un futuro no muy lejano."
